...
const fetchImage = async (url: string): Promise<ArrayBuffer | null> => {
    try {
        ...
        console.warn('Failed to fetch image for DOCX export (timeout or error):', url);
        return null;
    }
};
 ...
 */
const processInlineChildren = async (element: Node, config: StyleConfig, linkConfig: { color: string, underline: boolean }): Promise<any[]> => {
    const children: any[] = [];
    ...
    for (const child of Array.from(element.childNodes)) {
        if (child.nodeType === Node.TEXT_NODE) {
            ...
            if (rawLatex) {
                ...
                // For DOCX, we render the LaTeX source as a distinct style (Cambria Math if possible, or Monospace)
                children.push(new TextRun({
                    text: decodedLatex,
                    font: "Cambria Math", // Try to use a math font
                    size: config.fontSize * 2,
                    color: config.color.replace('#', ''), // Inherit color
                    italics: true, // Make it italic to distinguish
                }));
            ...
            if (tagName === 'img') {
                ...
                if (src) {
                    ...
                    if (buffer) {
                        ...
                        children.push(new TextRun({ text: `[Image: ${el.getAttribute('alt') || 'Image'}]` }));
                    }
                }
                continue;
            }
            ...
            if (tagName === 'a') {
                ...
                const linkChildren = await processInlineChildren(el, { ...config, color: linkConfig.color, underline: linkConfig.underline }, linkConfig);
            ...
            const nestedChildren = await processInlineChildren(el, {
                ...config,
                bold: isBold,
                italic: isItalic,
 ...
 */
export const generateDocxBlob = async (htmlContent: string, config: DocumentConfig): Promise<Blob> => {
  ...
  for (const node of nodes) {
         ...
         docxChildren.push(new Paragraph({
             ...
             children: [
                 new TextRun({
                     text: decodedLatex,
                     font: "Cambria Math",
                     size: config.p.fontSize * 2 + 4, // Slightly larger
                     italics: true
                 })
             ]
         }));
    ...
    if (['h1', 'h2', 'h3'].includes(tagName)) {
        ...
        if (tagName === 'h2') { style = config.h2; headingLevel = HeadingLevel.HEADING_2; }
        if (tagName === 'h3') { style = config.h3; headingLevel = HeadingLevel.HEADING_3; }
        ...
        docxChildren.push(new Paragraph({
            ...
            spacing: { before: style.marginTop * 20, after: style.marginBottom * 20 },
    ...
    // 2. PARAGRAPHS
        ...
        if (runs.length > 0) {
            docxChildren.push(new Paragraph({
                ...
                spacing: { before: config.p.marginTop * 20, after: config.p.marginBottom * 20 },
                children: runs
            }));
        }
    }
    // 3. BLOCKQUOTES
        ...
        docxChildren.push(new Paragraph({
            ...
            spacing: { before: config.quote.marginTop * 20, after: config.quote.marginBottom * 20 },
            indent: { left: 720 }, // Indent ~0.5 inch
            border: {
                left: { color: "auto", space: 120, style: BorderStyle.SINGLE, size: 6 }
            },
            children: runs
        }));
    }
    // 4. CODE BLOCKS (PRE)
        ...
        docxChildren.push(new Paragraph({
            ...
            spacing: { before: config.code.marginTop * 20, after: config.code.marginBottom * 20 },
    ...
    // 6. TABLES
        ...
        const docxRows = await Promise.all(rows.map(async (tr) => {
            const cells = Array.from(tr.querySelectorAll('th, td'));
            const docxCells = await Promise.all(cells.map(async (td) => {
