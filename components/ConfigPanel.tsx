...
import { Settings, Type, AlignLeft, AlignCenter, AlignRight, AlignJustify, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="border border-gray-200 dark:border-slate-700 rounded-lg mb-3 overflow-hidden bg-white dark:bg-slate-800">
      ...
      {isOpen && (
        <div className="p-4 space-y-4">
          ...
          <div className="flex items-end gap-3">
             <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-xs font-mono text-slate-400">{value.color}</span>
              </div>
            </div>
            <div className="flex gap-1">
               <button 
                  onClick={() => update('bold', !value.bold)}
                  className={`p-2 rounded border ${value.bold ? 'bg-blue-100 border-blue-300 text-blue-700' : 'bg-white dark:bg-slate-700 border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200'}`}
               >
                 B
               </button>
               <button 
                  onClick={() => update('italic', !value.italic)}
                  className={`p-2 rounded border italic ${value.italic ? 'bg-blue-100 border-blue-300 text-blue-700' : 'bg-white dark:bg-slate-700 border-gray-200 dark:border-slate-600 text-slate-700 dark:text-slate-200'}`}
               >
                 I
               </button>
            </div>
          ...
          {/* Alignment */}
          <div>
             <label className="block text-xs font-medium text-slate-500 mb-2">Alignment</label>
             <div className="flex bg-gray-100 dark:bg-slate-900 p-1 rounded-md">
                {['left', 'center', 'right', 'justify'].map((align) => (
                  <button
                    key={align}
                    onClick={() => update('alignment', align)}
                    className={`flex-1 flex justify-center py-1.5 rounded text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 ${value.alignment === align ? 'bg-white dark:bg-slate-700 shadow-sm text-blue-600 dark:text-blue-400' : ''}`}
                  >
           ...
           <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Margin Top (pt)</label>
              <input 
                type="number" 
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Margin Bottom (pt)</label>
              <input 
                type="number" 
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-slate-900 border-l border-gray-200 dark:border-slate-800">
      <div className="p-4 border-b border-gray-200 dark:border-slate-800 flex items-center gap-2">
        ...
        <h2 className="font-bold text-slate-800 dark:text-white">Styles Config</h2>
      </div>
